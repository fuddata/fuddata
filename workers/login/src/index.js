...
export default {
  async fetch(request, env) {
    ...
    if (paramAction == "validate") {
      console.log("Validating auth token ID: " + paramToken);
      var { value, metadata } = await env.KV_SESSIONS.getWithMetadata(paramToken);
      if (value == null) {
      ...
      if (emailHash == value) {
        ...
        if (metadata.loggedIn == false) {
          await env.KV_SESSIONS.put(paramToken, emailHash, {
            metadata: {
              ipHash: ipHash,
              expirationTtl: 7200,
              loggedIn: true,
