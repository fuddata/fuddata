...
func GetLicenseStatus(company, app, apiUrl, pubKey string) (int, bool, int, string, error) {
	...
	if license != "" {
		...
		if err != nil {
			...
			return 0, false, licenseType, clientID, fmt.Errorf("verifying license: %s", err)
		}
	}
	...
	default:
		// Make installation working in offline by allowing one day trial period when application does not have access to internet.
		return -1, false, licenseType, clientID, fmt.Errorf("unhandled licensing status. Details: %s", err)
	}
